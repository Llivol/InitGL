...
void Game::init()
{
	...
	Transform& light_1_transform = ECS.createComponentForEntity<Transform>(ent_light_1);
	...
	Transform& light_2_transform = ECS.createComponentForEntity<Transform>(ent_light_2);
	light_2_transform.translate(1000.0f, 1000.0f, 1000.0f);
	light_2_comp.color = lm::vec3(0.0f, 1.0f, 0.0f);

	int ent_camera_1 = ECS.createEntity("Camera 1");
	Camera& camera_1_comp = ECS.createComponentForEntity<Camera>(ent_camera_1);
	ECS.getComponentFromEntity<Transform>(ent_camera_1).translate(0.0f, 0.0f, 3.0f);
	...
	int ent_camera_2 = ECS.createEntity("Camera 2");
	Camera& camera_2_comp = ECS.createComponentForEntity<Camera>(ent_camera_2);
	ECS.getComponentFromEntity<Transform>(ent_camera_2).translate(0.0f, -0.2f, 1.0f);
	camera_2_comp.position = lm::vec3(0.0f, -0.2f, 1.0f);
	camera_2_comp.forward = lm::vec3(0, 0, -1.0f);
